...
void DisplayAtConsole::printYoneticiListesi() {
    ...
    system("cls");
    if (count == 0) {
        std::cout<< "Goruntulenecek bir Yonetici listesi yok\n";
        exit(1);
        return;
    }
 ...
 void DisplayAtConsole::printSelectedSatirListesi(int x, int y) {
  ...
  int DisplayAtConsole::DisplayRandomSelectedNode(){
  ...
  void DisplayAtConsole::DeleteRandomSelectedNode(int randomIndex) {
